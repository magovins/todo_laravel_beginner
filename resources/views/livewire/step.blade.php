<div>
    ...
    <div class="flex justify-center py-2" wire:key="{{$step}}">
        <input type="text" name="stepName[]" class="py-1 px-2 border rounded" placeholder="{{'Describe Step '.($step)}}" />
        <span class="fas fa-times text-red-400 p-2" wire:click="decrement({{$step}})" />
    </div>
    @endforeach
</div>